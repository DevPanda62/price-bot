...
- **Kritik ürün doğrulama katmanı (gelişmiş tarama):** her taramada error (hatalı fiyat girişi) ve indirim adaylarından en kritik ürünler (önce error, sonra fiyatı değişen, sonra yeni giren; site başı en fazla 15, toplam 60) doğrudan ürün sayfasından tek istekle doğrulanır (`core/verify.py`). Ürün sayfasındaki fiyat listeden düşükse **anlık indirim** (sitenin listeye henüz yansıtmadığı indirim) olarak işaretlenir ve fiyat güncellenir; aynı ürün 15 dakikada bir (yeniden) kontrol edilir. Bu adım **proxy kullanmaz** — proxy yalnızca site zaten proxy/utm modundaysa devreye girer, böylece minimum proxy harcanır. Sonuçlar `stats.json` → `critical` alanında ve run özetinde görünür.
