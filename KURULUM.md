...
- **3 aşamalı tarama:** 1) proxysiz tarama (TLS taklidi + sitemap yedeği ile) → 2) sadece engellenen siteler proxy ile → 3) proxy de açılmazsa **Ultra Trick Mode** (saatte 1: öğrenilmiş/çoklu tarayıcı profili, HTTP/1.1 düşüşü, 2 farklı proxy grubu). Sitenin o an indirimli ürünü olmaması ("0 ürün") engel sayılmaz. Arama sayfasındaki ürünler üç katmandan çıkarılır: site seçici kalıpları → **JSON-LD** (`ItemList` → `Product` + `offers`, JS ile doldurulan marketler: mediamarkt, amazon, vatan) → genel kart taraması.
- **Pas geçilen siteler tamamen ölmez:** UTM de yanıt vermezse site `skipped` moduna geçer ama iki mekanizma onu canlı tutar:
  - **Akakce yedeği (son çare):** site pas geçilmeden hemen önce ve pas süresince her taramada `akakce.com/magaza/<site>.html` sayfalarından o siteyi satan ürünlerin indirim verileri çekilir (`scrapers/akakce.py`, `config.json` → `akakceFallback`). Ürünler `store="akakce"`, `query="fallback:<site>"` etiketiyle listeye girer, normal indirim analizinden geçer. Akakçe de bot korumalıysa aynı UTM/proxy katmanı kullanılır (`akakceFallback.useProxy`).
  - **Canlandırma:** pas geçilen site `skipRetryMinutes` (varsayılan 1440 dk = günde 1) sonra UTM kademesiyle yeniden denenir — site koruması zamana göre değişebilir, deneme anında açılırsa tarama normale döner.
...
- **Proxy kota koruması:** proxy modundaki bir site tam taramayı 5 dk yerine `proxyEveryMinutes` (30 dk) aralığında yapar (`proxy_last` damgası); doğrulama site başı 5 ürün, 30 dakikada bir tekrarlanır (`criticalPerSite` 5, `criticalRecheckMinutes` 30); UTM saatlik denemede en fazla 16 istek (4 profil × 2 grup × 2 http11). "0 ürün" sonucu da (proxy ile açılıp indirim bulunmaması) `proxy_last` damgası yazar — böylece her taramada yeniden proxy basılmaz. Böylece 7 Webshare hesabının (900 MB × 7 ≈ 6.3 GB/ay) tek engelli site tarafından tüketilmesi engellenir.
- **Kategori filtresi (beyaz liste):** yalnızca elektronik (telefon, televizyon, bilgisayar, tablet, oyun konsolu, giyilebilir teknoloji) ile **erkek/unisex** giyim ve ayakkabı ürünleri indexlenir. Kadın giyimi, tüm iç çamaşırı (sütyen, külot, boxer, atlet, slip vb.), kişisel bakım, kozmetik, oyuncak, ev gereçleri, yiyecek + **ilgili ürünler** (vitamin, takviye, hap, kapsül, ilaç, şurup) ve **yedek parça/tamir ürünleri** (ekran değişimi, koruyucu cam, tamir seti vb.) KESİNLİKLE taranmaz/indexlenmez. Filtre üç katmanlıdır: sitemap URL seçiminde, ürün sayfası çıkarımında ve `run.py` son kontrolünde uygulanır (`core/categories.py`). Listedeki "korunan" kategorilere uymayan her ürün (ör. kitap, mobilya) API'ye hiç girmez.
